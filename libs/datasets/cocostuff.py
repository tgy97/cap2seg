...
class _CocoStuff(data.Dataset):
    ...
    def __init__(
        ...
        self.cls_score =  cls_score # last 15 classes are novel
        # self.threshold = 0.0075
        # self.random_scale = 0.0005
    ...
    def _transform(self, image, label, with_novel = 0):
        ...
        return image, label
    def _select_novel_cls(self, c_embed):
    	pro = (c_embed * self.all_cls_embed).sum(axis = 1)
    	pro = np.exp(pro) / np.sum(np.exp(pro))
    	pro_novel = pro[-15:]
    	selected = np.argmax(pro_novel)
    	if pro_novel[selected] > (self.threshold + (np.random.rand()-0.5)*self.random_scale*2): # random in 0.007 ~ 0.008
    		#return selected  # 0~14 
    		return selected + 181-14 # 0 ~ 181
    	else:
    		return -1
    	pass
    ...
    def __getitem__(self, index):
        ...
        if self.preload:
            ...
            image, label = self._load_data(image_id)
            if not self.cls_score is None:
                score = self.cls_score[0][image_id[1]]
                loc = self.cls_score[1][image_id[1]]
                
                #selected = self._select_novel_cls(c_embed)

        if isinstance(index, list):
            image, label = self._transform(image, label, index[1])
        else:
            image, label = self._transform(image, label, False)
            return image.astype(np.float32), label.astype(np.int64), str(image_id[1])  # not operate

        if not self.cls_score is None:
            # label = label.astype(np.int64)
            # label_novel = np.full(label.shape, -1)
            # label_novel[np.where(label == -1)] = selected
            
            return image.astype(np.float32), label.astype(np.int64), score, loc
